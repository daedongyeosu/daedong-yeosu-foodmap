...
for (const handler of ["openYeosuGageGuide","openHolidayMedicalGuide","openPublicToiletGuide","openCarAccidentGuide","openUsedMarketGuide","openLocalNewsGuide"]) assert.match(app, new RegExp(`function ${handler}\\(\\)`));
for (const id of ["yeosuGageBtn","holidayMedicalBtn","publicToiletBtn","carAccidentBtn","usedMarketBtn","localNewsBtn"]) assert.match(app, new RegExp(`\\$\\('#${id}'\\)\\?\\.addEventListener`));
