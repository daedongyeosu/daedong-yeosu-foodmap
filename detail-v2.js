(()=>{
  ...
  detail=function(store){
    ...
    openModal(`<div class="store-detail-head"><h2>${esc(store.name)}</h2></div><img src="${esc(store.img)}" class="detail-photo" alt="${esc(store.name)}" onerror="this.src='assets/store1.jpg'">${quick}<p class="detail-meta">${esc(store.area)} · ${esc(store.cat)}</p><div class="detail-routes">${primary}${others}</div>`);
  };
})();