(()=>{
  ...
  function hardClose(){
    ...
    if(modal){modal.hidden=true;modal.className='modal';modal.style.removeProperty('--v7-store-scale');}
    ...
    try{
      ...
      if(history.state?.daedongModal)history.replaceState(null,'');
    }catch{}
  }

  /* 기존 닫기 함수들을 한 함수로 통일하여 X만 사라지고 팝업이 남는 현상을 차단 */
  ...
  function bindHero(){
    const track=document.querySelector('#heroTrack');
    if(!track||track.dataset.v7Bound==='1')return;
  ...
  function selectedCta(content){
    ...
    const store=stores.find(item=>String(item.id)===String(selected.storeId));
    const meta=window.DaedongAppBrowser?.meta?.[selected.key];
    const url=window.DaedongAppBrowser?.routeFor?.(store,selected.key);
  ...
  function fitStore(){
    ...
    if(!modal?.classList.contains('v7-store')||!card)return;
    card.style.setProperty('--v7-store-scale','1');
    requestAnimationFrame(()=>{
      const natural=card.scrollHeight||card.getBoundingClientRect().height;
      const available=Math.max(420,window.innerHeight-12);
      const scale=Math.min(1,available/Math.max(1,natural));
      card.style.setProperty('--v7-store-scale',String(Math.max(.72,Math.floor(scale*1000)/1000)));
    });
  }
  ...
  function classifyModal(){
    ...
    if(content.querySelector('.app-browser'))modal.classList.add('v7-sheet','v7-app');
    else if(content.querySelector('.all-category-sheet-v2'))modal.classList.add('v7-sheet','v7-category');
    else if(content.querySelector('.store-detail-head')&&content.querySelector('.detail-routes')){
      modal.classList.add('v7-store');
      content.querySelectorAll('.ux-detail-route-grid,.ux-detail-actions').forEach(unwrap);
      selectedCta(content);
      fitStore();
    }else{
      modal.classList.add('v7-sheet','v7-general');
      if(content.querySelector('.location-onboarding,.address-sheet,.postcode-sheet,.address-detail-sheet'))modal.classList.add('v7-location');
    }
  }
  ...
  if(typeof baseOpen==='function'){
    window.openModal=function(html){
      ...
      if(modal&&!modal.hidden){
        /* 현재 팝업을 같은 하나의 modal 안에서 교체하므로 중복 레이어가 생기지 않는다. */
      ...
      setTimeout(classifyModal,40);
    };
  }

  function createViewer(){
    let viewer=document.querySelector('#v7PhotoViewer');
    if(viewer)return viewer;
    viewer=document.createElement('div');viewer.id='v7PhotoViewer';viewer.hidden=true;
    viewer.innerHTML='<button class="photo-close">×</button><button class="photo-prev">‹</button><img alt="가게 사진 전체화면"><button class="photo-next">›</button><span class="photo-count"></span>';
    document.body.append(viewer);
    viewer.querySelector('.photo-close').onclick=()=>viewer.hidden=true;
    viewer.querySelector('.photo-prev').onclick=()=>moveViewer(-1);
    viewer.querySelector('.photo-next').onclick=()=>moveViewer(1);
    return viewer;
  }
  const viewerState={photos:[],index:0};
  function renderViewer(){const viewer=createViewer();viewer.querySelector('img').src=viewerState.photos[viewerState.index]||'';viewer.querySelector('.photo-count').textContent=`${viewerState.index+1} / ${viewerState.photos.length}`;viewer.querySelector('.photo-prev').hidden=viewerState.photos.length<2;viewer.querySelector('.photo-next').hidden=viewerState.photos.length<2;}
  ...
  document.addEventListener('pointerup',event=>{
    if(event.target.closest('#modal .modal-close')){event.preventDefault();event.stopImmediatePropagation();hardClose();}
  },true);
  document.addEventListener('click',event=>{
    if(event.target.closest('#modal .modal-close')){event.preventDefault();event.stopImmediatePropagation();hardClose();return;}
    const manual=event.target.closest('[data-location-manual]');
    if(manual){event.preventDefault();event.stopImmediatePropagation();window.DaedongOpenManualAddress?.();return;}
    const current=event.target.closest('[data-location-current]');
    if(current&&!current.disabled){event.preventDefault();event.stopImmediatePropagation();hardClose();setTimeout(()=>typeof useCurrentLocation==='function'&&useCurrentLocation(),80);return;}
    const photo=event.target.closest('.v7-store .detail-gallery-slide img');
    if(photo){event.preventDefault();event.stopPropagation();openViewer(photo);return;}
    const search=event.target.closest('[data-campaign-search]');
    if(search){const input=document.querySelector('#mainSearch');if(input)input.value=search.dataset.campaignSearch;hardClose();setTimeout(()=>document.querySelector('#searchBtn')?.click(),80);}
  },true);

  window.addEventListener('popstate',()=>{const modal=document.querySelector('#modal');if(modal&&!modal.hidden)hardClose();});
  ...
  function apply(){
    ...
    const build=document.querySelector('.build-mark');if(build)build.textContent='build DB790-single-modal-v7';
  }
