(()=>{
  ...
  function exactClickable(content,needle){
    return [...content.querySelectorAll('button,a')].find(node=>text(node.textContent).includes(needle));
  }

  function compactDetailStructure(content){
    if(content.dataset.uxCompacted==='1') return;
    const routeWords=['가게바로주문','먹깨비','땡겨요','다른주문방법보기'];
    const routes=routeWords.map(word=>exactClickable(content,word)).filter(Boolean);
    if(routes.length>=3){
      const routeGrid=document.createElement('div');
      routeGrid.className='ux-detail-route-grid';
      routes[0].parentNode.insertBefore(routeGrid,routes[0]);
      routes.forEach(node=>routeGrid.append(node));
    }
    const favorite=exactClickable(content,'찜하기');
    const info=exactClickable(content,'정보수정요청');
    if(favorite&&info){
      const actions=document.createElement('div');
      actions.className='ux-detail-actions';
      favorite.parentNode.insertBefore(actions,favorite);
      actions.append(favorite,info);
    }
    content.dataset.uxCompacted='1';
  }

  function fitStoreDetail(){
    const modal=document.querySelector('#modal');
    const content=document.querySelector('#modalContent');
    const card=modal?.querySelector('.modal-card');
    if(!modal||!content||!card||modal.hidden){
      if(modal) modal.classList.remove('store-detail-compact');
      if(card) card.style.zoom='';
      return;
    }
    const value=text(content.textContent);
    const isDetail=value.includes('가게바로주문')&&value.includes('찜하기')&&value.includes('정보수정요청');
    modal.classList.toggle('store-detail-compact',isDetail);
    if(!isDetail){card.style.zoom='';return;}
    compactDetailStructure(content);
    card.style.zoom='1';
    requestAnimationFrame(()=>{
      const available=Math.max(420,window.innerHeight-16);
      const natural=card.getBoundingClientRect().height;
      const scale=Math.min(1,available/Math.max(1,natural));
      card.style.zoom=String(Math.max(.56,Math.floor(scale*1000)/1000));
    });
  }

  function updateBuild(){
    const build=document.querySelector('.build-mark');
    if(build) build.textContent='build DB790-mobile-ux2';
  }
