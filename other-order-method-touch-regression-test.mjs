...
const productionWorkflow = read('./.github/workflows/preview-api-client-checks.yml');
...
assert.match(rc3, /other\.dataset\.rc3LastTouchActivation[\s\S]*?stopImmediatePropagation/, '터치 직후 ghost click은 같은 주문방법 버튼에서만 차단되어야 합니다.');
assert.doesNotMatch(rc3, /rc3OrderMethodsGhostClickUntil/, '한 버튼의 ghost click 차단이 화면 전체 클릭을 막으면 안 됩니다.');
assert.match(rc3, /const rc3BoundOrderMethodsTriggers = new WeakSet\(\)/, '복원된 DOM의 실제 이벤트 연결 상태를 추적해야 합니다.');
assert.match(rc3, /window\.daedongRestoreModalInteractions = rc3RestoreModalInteractions/, '외부 앱 복귀 후 터치 이벤트 재연결 경로가 없습니다.');
...
assert.match(rc3, /orderAnchor\?\.insertAdjacentHTML[\s\S]*?rc3BindOrderMethodsTrigger\(detail\);/, '가게 상세를 그린 뒤 직접 동작 보조 경로가 연결되지 않았습니다.');
...
assert.match(productionWorkflow, /node scripts\/browser-other-order-method-touch\.mjs/, '운영 PR에서 주문방법 모바일 터치 검사를 실행하지 않습니다.');
