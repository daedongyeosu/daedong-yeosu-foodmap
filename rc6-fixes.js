...
function rc6ResolveHeroCampaignStoreId(id){
 return String(rc6HeroCampaignForEntryStoreId(id)?.storeId||'');
}
function rc6RequestedHeroCampaign(){
 const params=new URLSearchParams(location.search),entryStoreId=params.get('hero')||params.get('store');
 const campaign=rc6HeroCampaignForEntryStoreId(entryStoreId);if(!campaign)return null;
 const storeId=String(campaign.storeId||'');
...
const rc6HeroDataPromise=Promise.all([fetchJson('data/store-coordinates.json',{}),fetchJson('data/banner-targets.json?v=notion-banner-15',{}),fetchJson('data/store-priority.json',{}),fetchJson('data/hero-campaigns.json?v=store-campaign-nine-3',{campaigns:{},virtualStores:{}}),rc6LoadRainMode()]);
