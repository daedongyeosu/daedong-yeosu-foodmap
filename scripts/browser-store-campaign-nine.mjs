...
// Keep both campaign-owned virtual stores absent from the base fixture.
// Bbungtteurak must still use its approved fallback, while Tamnaneun's preserved
// virtual-store data must not recreate a customer-hidden store.
...
try {
  for (const entry of manifest.campaigns) {
    ...
    if (entry.storeId === 'aa0a00258c22f377') {
      await page.screenshot({ path: path.join(output, 'browser-ppongtteurak-campaign.png'), fullPage: false });
    }
    await page.close();
  }
