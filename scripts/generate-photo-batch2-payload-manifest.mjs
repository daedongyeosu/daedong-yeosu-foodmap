...
const manifest={version:5,generatedAt:new Date().toISOString(),algorithm:'SHA-256',sourceOfTruth:'actual-agent-main-unification-files',fileCount:entries.length,files:entries};
fs.writeFileSync('payload-manifest.json',JSON.stringify(manifest,null,2)+'\n');
const sums=[...entries,{path:'payload-manifest.json',sha256:sha('payload-manifest.json')}].map(item=>`${item.sha256}  ${item.path}`).join('\n')+'\n';
