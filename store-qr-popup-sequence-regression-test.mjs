...
assert.match(html, /const dedicatedEntryStoreId\s*=\s*String\([\s\S]*?searchParams\.get\('hero'\)[\s\S]*?searchParams\.get\('store'\)[\s\S]*?window\.daedongDedicatedEntryStoreId\s*=\s*dedicatedEntryStoreId/,
