...
assert.match(app, /selector: '#modal \.modal-close'[\s\S]{0,180}hardClose\(\{userInitiated: true\}\)/,
  '가게 QR 닫힘 표시는 고객이 X를 직접 눌렀을 때만 남겨야 합니다.');
assert.match(rc2, /options\.userInitiated === true && !options\.fromPop && modalDepth <= 1/,
  '주문앱 이동·복귀 중 발생한 프로그램 닫힘을 QR 팝업 닫기로 오인하면 안 됩니다.');
assert.match(rc2, /if \(options\.userInitiated === true && !options\.fromPop && !sharedEntryConsumed\)/);
